...
func (c *ScrapeConfig) Validate(globalConfig GlobalConfig) error {
	...
	default:
		return fmt.Errorf("unknown global name validation method specified, must be either 'legacy' or 'utf8', got %v", globalConfig.MetricNameValidationScheme)
	}
	...
	default:
		return fmt.Errorf("unknown global name escaping method specified, must be one of '%s', '%s', '%s', or '%s', got %v", model.AllowUTF8, model.EscapeUnderscores, model.EscapeDots, model.EscapeValues, globalConfig.MetricNameValidationScheme)
	}
